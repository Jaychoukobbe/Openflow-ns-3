...
main (int argc, char *argv[])
{
  ...
  for (size_t i = 0; i < hosts.GetN (); i+=2)
    {
      ...
      NodeContainer pair1 (hosts.Get (i+1), switches.Get(i));
...
//      csmaHelper.EnablePcap ("host", hostDevices);
  	...
  	AnimationInterface::SetConstantPosition(hosts.Get(0),14.5,15);
  	AnimationInterface::SetConstantPosition(hosts.Get(0),65.5,15);
  	AnimationInterface::SetConstantPosition(hosts.Get(0),30,70);
  ...
  pAnim=new AnimationInterface("my-of13-group.xml");
	...
	void
		HandshakeSuccessful (Ptr<const RemoteSwitch> swtch)
		{
			if((int)swtch->GetDpId()==1){
				DpctlExecute (swtch, "group-mod cmd=add,type=all,group=1 weight=0,port=any,group=any  output=3 weight=0,port=any,group=any output=4");
				printf("%d",(int)swtch->GetDpId());
				DpctlExecute (swtch, "flow-mod cmd=add,table=0,hard=2,flags=0x0001,prio=2 in_port=2 apply:group=1");
				DpctlExecute (swtch, "flow-mod cmd=add,table=0,hard=2,flags=0x0001,prio=3 in_port=1 apply:group=1");
			}
			if((int)swtch->GetDpId()==2){
				DpctlExecute (swtch, "group-mod cmd=add,type=all,group=1 weight=0,port=any,group=any  output=1 weight=0,port=any,group=any output=4");
				printf("%d",(int)swtch->GetDpId());
				printf("%d",(int)swtch->GetDpId());
				DpctlExecute (swtch, "flow-mod cmd=add,table=0,flags=0x0001,prio=2 in_port=2 write:output=3");
				DpctlExecute (swtch, "flow-mod cmd=add,table=0,flags=0x0001,prio=1 in_port=3 write:group=1");
			}
			if((int)swtch->GetDpId()==3){

				printf("%d",(int)swtch->GetDpId());
				DpctlExecute (swtch, "flow-mod cmd=add,table=0,hard=2,flags=0x0001,prio=2 in_port=3 write:output=1");
				DpctlExecute (swtch, "flow-mod cmd=add,table=0,hard=2,flags=0x0001,prio=1 in_port=4 write:output=2");
			}
		}
		...
		ofl_err HandleFlowRemoved (
		    ...
		    uint32_t xid){
//			if((int)msg->stats->hard_timeout==1){
//				DpctlExecute (swtch, "stats-flow");
//				DpctlExecute (swtch, "flow-mod cmd=add,table=0,hard=1,flags=0x0001,prio=0 ");
//			}
...
//		void
//		ReceiveFromSwitch (Ptr<Packet> packet, Address from)
//		{
