...
main (int argc, char *argv[])
{
  ...
  Ptr<OFSwitch13InternalHelper> of13Helper2 = CreateObject<OFSwitch13InternalHelper> ();
  Ptr<Controller0> ctrl0 = CreateObject<Controller0> ();
  Ptr<Controller1> ctrl1 = CreateObject<Controller1> ();
  Ptr<Controller2> ctrl2 = CreateObject<Controller2> ();
  ...
  of13Helper1->InstallController (controllers.Get (1), ctrl1);
  of13Helper1->InstallSwitch (switches.Get(1), switchPorts[1]);
  of13Helper1->CreateOpenFlowChannels ();
  of13Helper2->InstallController (controllers.Get (2), ctrl2);
  of13Helper2->InstallSwitch (switches.Get(2), switchPorts[2]);
  of13Helper2->CreateOpenFlowChannels ();
...
//      csmaHelper.EnablePcap ("host", hostDevices);
    ...
    AnimationInterface::SetConstantPosition(controllers.Get(0),32,35);
    AnimationInterface::SetConstantPosition(controllers.Get(1),48,35);
    AnimationInterface::SetConstantPosition(controllers.Get(2),40,50);
...
protected:
  // Inherited from OFSwitch13Controller
	...
	{
//		  DpctlExecute (swtch, "group-mod cmd=add,type=all,group=1 weight=0,port=any,group=1  output=ctrl weight=0,port=any,group=1 output=1");
//		  DpctlExecute (swtch, "group-mod cmd=add,type=all,group=2 weight=0,port=any,group=2  output=ctrl weight=0,port=any,group=2 output=2");
//		  DpctlExecute (swtch, "group-mod cmd=add,type=all,group=3 weight=0,port=any,group=3  output=ctrl weight=0,port=any,group=3 output=3");
//	  DpctlExecute (swtch, "flow-mod cmd=add,table=0,hard=2,flags=0x0001,prio=2 in_port=2 apply:group=1");
//	  DpctlExecute (swtch, "flow-mod cmd=add,table=0,hard=2,flags=0x0001,prio=3 in_port=1 apply:group=2");
...
class Controller2 : public OFSwitch13Controller
{
