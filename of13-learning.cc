...
main (int argc, char *argv[])
{
  ...
  pair = NodeContainer (hosts.Get (2), switches.Get (2));
  pairDevs = csmaHelper.Install (pair);
  hostDevices.Add (pairDevs.Get (0));
  ...
  switchPorts [1].Add (pairDevs.Get (1));
  pair = NodeContainer (switches.Get (2), switches.Get (1));
  pairDevs = csmaHelper.Install (pair);
  ...
  AnimationInterface::SetConstantPosition(controllerNode,40,40);

  // Run the simulation
  Simulator::Stop (Seconds (simTime));
  pAnim=new AnimationInterface("my-of13-lerning.xml");
